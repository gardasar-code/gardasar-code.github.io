...
class Di2FieldView extends WatchUi.DataField {
    ...
    // Диагностический экран создаём по требованию: в обычной работе оверлей выключен,
    // и держать объект (со своими ссылками) незачем.
    private var _diag as Di2DiagScreen?;
    private function diagScreen() as Di2DiagScreen {
        if (_diag == null) {
            _diag = new Di2DiagScreen(_state);
        }
        return _diag as Di2DiagScreen;
    }
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        if ((DEBUG_OVERLAY || (_state != null && _state.diagOverlay)) && _state != null) {
            diagScreen().drawDiagScreen(dc, w, h, fg, bg, fade);
            return;
        }
