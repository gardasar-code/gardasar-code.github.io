...
# Имя результата — с версией (в Finder видно, что за сборка), но ТОЧКИ ЗАМЕНЕНЫ НА
# ПОДЧЁРКИВАНИЯ, и это не косметика.
#
# Устройство пишет лог в файл, имя которого совпадает с именем .prg. Проверено на
# железе: с `Di2App-0.0.49-diag.prg` лог не пишется НИ в `Di2App-0.0.49-diag.TXT`,
# ни в нормализованный `Di2App-0_0_49-diag.TXT` (при этом счётчик log= на diag-экране
# показывал 25 записанных строк — то есть println звался, а файла для него не нашлось).
# С именем `Di2App-0_0_45-diag.prg` тот же код писал лог штатно. Точек в имени быть
# не должно — тогда .prg и .TXT совпадают буквально.
NAME="Di2App-${VERSION//./_}-diag"
OUT="bin/${NAME}.prg"
LOGNAME="${NAME}.TXT"
...
# Готовим файл лога с ПРАВИЛЬНЫМ именем: создавать его вручную — постоянный источник
# ошибок (имя обязано совпадать с именем .prg, иначе вывод молча теряется).
#
# Файл НЕ пустой намеренно: на FAT файл нулевой длины не занимает ни одного кластера,
# и наблюдалось, что устройство в такой файл не дописывает (счётчик log= на diag-экране
# растёт, а файл после снятия пуст). Одна строка-заголовок даёт файлу реальный кластер.
printf '# Di2 Field diagnostic log\n' > "bin/$LOGNAME"

echo "✓ $OUT      -> GARMIN/APPS/"
echo "✓ bin/$LOGNAME -> GARMIN/APPS/LOGS/   (файл лога с нужным именем, устройство дописывает в него)"
